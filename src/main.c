...
const mb_parameter_descriptor_t device_parameters[] = {
    {CID_INP_DATA_0,            // cid
     ...
     PAR_PERMS_READ},
    // {CID_INP_DATA_0, STR("Data_channel_0"), STR("Volts"), MB_DEVICE_ADDR1, MB_PARAM_HOLDING, 35121, 1,
    //  INPUT_OFFSET(input_data0), PARAM_TYPE_FLOAT, 4, OPTS(-10, 10, 1), PAR_PERMS_READ_WRITE_TRIGGER},
}; // parametr access type
...
static void master_operation_func(void *arg)
{
    ...
    while (true)
    // while (uint16_t retry = 0; retry <= MASTER_MAX_RETRY && (!alarm_state); retry++)
    {
...
static esp_err_t master_init(void)
{
    ...
    err = uart_set_pin(MB_PORT_NUM, tx_pin, rx_pin,
